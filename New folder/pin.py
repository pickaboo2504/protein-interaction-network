...
from proteingraph.resi_atoms import (
    AA_RING_ATOMS,
    AROMATIC_RESIS,
    BACKBONE_ATOMS,
    BOND_TYPES,
...
def pdb2df(path: Path) -> nx.Graph:
    """
    Parse the PDB file as a pandas DataFrame object.
...
def compute_chain_pos_aa_mapping(pdb_df):
    """Compute the mapping: chain -> position -> aa."""
...
def get_ring_centroids(ring_atom_df):
    ...
    Computes the ring centroids for each amino acid's aromatic ring atoms.
    ...
    Parameters:
    ===========
    - ring_atom_df: a dataframe computed using get_ring_atoms, containing columns
                    'node_id', 'x_coord', 'y_coord', 'z_coord'.

    Returns:
    ========
    - centroid_df: a dataframe containing the centroid coordinates (mean x, y, z)
                    of the ring atoms of each residue.
    """
    numeric_cols = ['x_coord', 'y_coord', 'z_coord']
    
    # Ensure numeric types for coordinates
    ring_atom_df[numeric_cols] = ring_atom_df[numeric_cols].apply(pd.to_numeric, errors='coerce')
    
    # Drop rows with NaN values in numeric columns
    ring_atom_df.dropna(subset=numeric_cols, inplace=True)
    
    # Calculate centroids by grouping by 'node_id' and computing mean
    centroid_df = (
